    ...
    <section className="card w-full max-w-lg p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="category-form-title">
      ...
      <form action={formAction} className="mt-6 space-y-4">
        <label className="block text-sm font-semibold">Tên danh mục
          <input className="mt-1.5 w-full rounded-lg border border-gray-300 px-3 py-2.5 outline-none focus:border-[#18181b]" name="name" value={name} onChange={(event) => { const value = event.target.value; setName(value); if (!manualSlug) setSlug(slugify(value)); }} required maxLength={80}/>
        ...
        <label className="block text-sm font-semibold">Đường dẫn danh mục
          <div className="mt-1.5 flex items-center rounded-lg border border-gray-300 bg-white focus-within:border-[#18181b]"><span className="pl-3 text-sm text-gray-400">/danh-muc/</span><input className="min-w-0 flex-1 rounded-lg px-1 py-2.5 outline-none" name="slug" value={slug} onChange={(event) => { setManualSlug(true); setSlug(slugify(event.target.value)); }} required maxLength={100}/></div>
        ...
        <label className="block text-sm font-semibold">Mô tả <span className="font-normal text-gray-400">(không bắt buộc)</span>
          <textarea className="mt-1.5 min-h-24 w-full resize-y rounded-lg border border-gray-300 px-3 py-2.5 outline-none focus:border-[#18181b]" name="description" defaultValue={category?.description ?? ""} maxLength={300}/>
...
export default function CategoryManager({ categories }: { categories: Category[] }) {
  ...
  return <section className="card overflow-hidden">
    <div className="flex flex-col justify-between gap-4 border-b p-5 sm:flex-row sm:items-center">
      <div><h2 className="flex items-center gap-2 text-xl font-bold"><Tags className="text-[#18181b]"/>Danh mục ({categories.length})</h2><p className="mt-1 text-sm text-gray-500">{categories.reduce((sum, item) => sum + item.productCount, 0)} sản phẩm đã được phân loại</p></div>
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[720px] text-left"><thead className="bg-gray-50 text-sm text-gray-600"><tr><th className="px-5 py-3">Tên danh mục</th><th className="px-5 py-3">Đường dẫn</th><th className="px-5 py-3">Mô tả</th><th className="px-5 py-3 text-center">Sản phẩm</th><th className="px-5 py-3 text-right">Thao tác</th></tr></thead>
        <tbody>{filtered.map(category => <tr className="border-t hover:bg-slate-50" key={category.id}><td className="px-5 py-4 font-semibold">{category.name}</td><td className="px-5 py-4"><code className="rounded bg-slate-100 px-2 py-1 text-sm">{category.slug}</code></td><td className="max-w-xs truncate px-5 py-4 text-sm text-gray-500">{category.description || "—"}</td><td className="px-5 py-4 text-center"><span className="rounded-full bg-zinc-100 px-2.5 py-1 text-sm font-semibold text-[#18181b]">{category.productCount}</span></td><td className="px-5 py-4"><div className="flex justify-end gap-1"><button className="rounded-lg p-2 text-[#18181b] hover:bg-zinc-100" onClick={() => setFormCategory(category)} title="Sửa danh mục" aria-label={`Sửa ${category.name}`}><Pencil size={18}/></button><DeleteButton category={category}/></div></td></tr>)}</tbody></table>

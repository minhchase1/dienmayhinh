...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const sampleProducts = query ? products.filter((product) => `${product.name} ${product.brand} ${product.sku} ${product.category} ${categoryNames.get(product.category) ?? ""}`.toLocaleLowerCase("vi").includes(normalizedQuery) && !databaseSlugs.has(product.slug)) : [];
